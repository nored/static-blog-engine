...
ArticleAnimator.createPost = function(opts, callback) {
  ...
  var index =
    type == "next"
      ? this.nextPostIndex(this.currentPostIndex)
...
ArticleAnimator.contentizeElement = function($el, d) {
  $el
    .find(".big-image")
    .css({ backgroundImage: "url(images/" + d.image + ")" });
...
ArticleAnimator.bindWindowScroll = function() {
  ...
  $window.on("mousewheel", function(ev) {
    if (!self.canScroll) ev.preventDefault();
  });
};
...
async function getPostCount() {
  ...
  ArticleAnimator.currentPostIndex = index;
  ArticleAnimator.postCount = index;
}
...
function getURLIndex() {
  return parseInt(
    (history.state && history.state.index) ||
      ...
      ArticleAnimator.currentPostIndex
  );
}
...
$(document).ready(function() {
  ...
  $.when(getPostCount()).then(function() {
